                ...
                <div class="row px-3">
                    ...
                    <div class="col-3 text-right ml-auto">
                        <div class="input-group input-group-sm mb-3">
                            ...
                            <input type="text" class="form-control border-right-0 searching" v-model="params.searchProvinces" placeholder="Search staffs..." aria-label="Search..." style="" aria-describedby="basic-addon2">
                ...
                <div class="row px-3 justify-content-start">
                    <div class="col">
                        <table class="table text-sm">
                                ...
                                <tr class="color-6-red">
                                    <th role="button" scope="col" class="pt-3 pb-1 border-0">
                                        <span class="d-flex justify-content-between align-items-center" @click="sort('name')">
                                            Province
                                            <i v-if="params.searchProvinces === null && params.field === null && params.direction === null" class="w-4 h-4 fas fa-sort"></i>
                            ...
                            <tbody class="nomatches" hidden="true">
                                <tr>
                                    <td colspan="2" class="text-center" v-if="ProvinceList.data.length == 0">
                            ...
                            <tbody >
                                <tr class="color-6-red" v-for="prov in ProvinceList.data" :key="prov">
                                    <td class="text-wrap text-sm" style="max-width: 150px;" v-html="prov.name"></td>
                                    <td class="text-wrap text-sm" style="max-width: 150px;">
                                        <a @click="viewCity(prov.id)" data-toggle="modal" data-target="#usersProfileModal" href="#" class="badge badge-pill badge-custom-yellow text-dark text-decoration-none float-left text-sm">View <i class="fas fa-eye"></i> </a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="col-12">
                        <pagination :links="ProvinceList.links" :current_page="ProvinceList.current_page" :prev_url="ProvinceList.prev_page_url" :next_url="ProvinceList.next_page_url" :total_page="ProvinceList.last_page" :path="ProvinceList.path"></pagination>
                    </div>
                </div>
            </section>
        </div>
...
<script>
    ...
    export default {
        ...
        props: [ 'UsersDetails', 'ProvinceList', 'Filters', 'UserRoles' ],
        data() {
            return {
                params:{
                    searchProvinces: this.Filters.searchProvinces,
            ...
            params: {
                handler(){
                    setTimeout(function () {
                        ...
                        this.$inertia.get(this.route(this.UserRoles[0]+'.ViewProvinces'), params, {
                            replace: true,
                            preserveState: false,
                            onSuccess: (res) => {
                                console.log("ViewProvinces");
                                // this.searching_data = res.props.Symptoms.data;
                                if(res.props.ProvinceList.data.length == 0){
